...
// source: table redeemed_tickets

package redeemedticket
...
func (q Query) IDEq(v string) Query               { return q.Where(ID.Eq(v)) }
func (q Query) IDNotEq(v string) Query            { return q.Where(ID.NotEq(v)) }
func (q Query) IDGt(v string) Query               { return q.Where(ID.Gt(v)) }
func (q Query) IDGte(v string) Query              { return q.Where(ID.Gte(v)) }
func (q Query) IDLt(v string) Query               { return q.Where(ID.Lt(v)) }
func (q Query) IDLte(v string) Query              { return q.Where(ID.Lte(v)) }
func (q Query) IDLike(v string) Query             { return q.Where(ID.Like(v)) }
func (q Query) IDILike(v string) Query            { return q.Where(ID.ILike(v)) }
func (q Query) IDIn(v ...string) Query            { return q.Where(ID.In(v...)) }
func (q Query) IDNotIn(v ...string) Query         { return q.Where(ID.NotIn(v...)) }
...
func (q Query) RedeemedAtEq(v time.Time) Query    { return q.Where(RedeemedAt.Eq(v)) }
func (q Query) RedeemedAtNotEq(v time.Time) Query { return q.Where(RedeemedAt.NotEq(v)) }
func (q Query) RedeemedAtGt(v time.Time) Query    { return q.Where(RedeemedAt.Gt(v)) }
func (q Query) RedeemedAtGte(v time.Time) Query   { return q.Where(RedeemedAt.Gte(v)) }
func (q Query) RedeemedAtLt(v time.Time) Query    { return q.Where(RedeemedAt.Lt(v)) }
func (q Query) RedeemedAtLte(v time.Time) Query   { return q.Where(RedeemedAt.Lte(v)) }
...
const selectPrefix = `SELECT "id", "email", "target", "principal", "redeemed_at", "expires_at" FROM "redeemed_tickets"`
const countPrefix = `SELECT count(*) FROM "redeemed_tickets"`
const existsPrefix = `SELECT 1 FROM "redeemed_tickets"`
...
var orderTable = [nCols][4]string{
	...
	{ // redeemed_at
		"\"redeemed_at\"",
		"\"redeemed_at\" DESC",
		"\"redeemed_at\" ASC NULLS FIRST",
		"\"redeemed_at\" DESC NULLS LAST",
	},
	{ // expires_at
		"\"expires_at\"",
		"\"expires_at\" DESC",
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	r.ID = sl.Str(rv[0])
	r.Email = sl.Str(rv[1])
	r.Target = sl.Str(rv[2])
	r.Principal = sl.Str(rv[3])
	r.RedeemedAt = runtime.Timestamptz(rv[4])
...
const insertSQL = `INSERT INTO "redeemed_tickets" ("id", "email", "target", "principal", "redeemed_at", "expires_at") VALUES ($1, $2, $3, $4, $5, $6) RETURNING "id", "email", "target", "principal", "redeemed_at", "expires_at"`

const updatePrefix = `UPDATE "redeemed_tickets" SET `
const deletePrefix = `DELETE FROM "redeemed_tickets"`
...
const (
	...
	dRedeemedAt uint64 = 1 << 3
	dExpiresAt  uint64 = 1 << 4
)
...
const (
	...
	iRedeemedAt uint64 = 1 << 4
	iExpiresAt  uint64 = 1 << 5
)
...
const insPrefix = "INSERT INTO \"redeemed_tickets\""
const insReturning = " RETURNING \"id\", \"email\", \"target\", \"principal\", \"redeemed_at\", \"expires_at\""
...
func (m *Mut) SetRedeemedAt(v time.Time) {
	m.row.RedeemedAt = v
	m.dirty |= dRedeemedAt
}
...
func (n *Ins) SetRedeemedAt(v time.Time) {
	n.row.RedeemedAt = v
	n.set |= iRedeemedAt
}
...
var assignFor = map[string]string{
	...
	"redeemed_at": "\"redeemed_at\" = EXCLUDED.\"redeemed_at\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 4:
			args = append(args, n.row.RedeemedAt)
		case 5:
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "redeemed_tickets", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 3:
			args = append(args, m.row.RedeemedAt)
		case 4:
...
func DeleteOp(iD string) runtime.BatchOp {
...
func Delete(ctx context.Context, ex runtime.Executor, iD string) error {
