...
// source: table asset_assignments
...
func (q Query) GrantedByEq(v string) Query              { return q.Where(GrantedBy.Eq(v)) }
func (q Query) GrantedByNotEq(v string) Query           { return q.Where(GrantedBy.NotEq(v)) }
func (q Query) GrantedByGt(v string) Query              { return q.Where(GrantedBy.Gt(v)) }
func (q Query) GrantedByGte(v string) Query             { return q.Where(GrantedBy.Gte(v)) }
func (q Query) GrantedByLt(v string) Query              { return q.Where(GrantedBy.Lt(v)) }
func (q Query) GrantedByLte(v string) Query             { return q.Where(GrantedBy.Lte(v)) }
func (q Query) GrantedByLike(v string) Query            { return q.Where(GrantedBy.Like(v)) }
func (q Query) GrantedByILike(v string) Query           { return q.Where(GrantedBy.ILike(v)) }
func (q Query) GrantedByIn(v ...string) Query           { return q.Where(GrantedBy.In(v...)) }
func (q Query) GrantedByNotIn(v ...string) Query        { return q.Where(GrantedBy.NotIn(v...)) }
func (q Query) GrantedAtEq(v time.Time) Query           { return q.Where(GrantedAt.Eq(v)) }
func (q Query) GrantedAtNotEq(v time.Time) Query        { return q.Where(GrantedAt.NotEq(v)) }
func (q Query) GrantedAtGt(v time.Time) Query           { return q.Where(GrantedAt.Gt(v)) }
func (q Query) GrantedAtGte(v time.Time) Query          { return q.Where(GrantedAt.Gte(v)) }
func (q Query) GrantedAtLt(v time.Time) Query           { return q.Where(GrantedAt.Lt(v)) }
func (q Query) GrantedAtLte(v time.Time) Query          { return q.Where(GrantedAt.Lte(v)) }

const selectPrefix = `SELECT "asset_id", "user_email", "principals", "granted_by", "granted_at" FROM "asset_assignments"`
const countPrefix = `SELECT count(*) FROM "asset_assignments"`
const existsPrefix = `SELECT 1 FROM "asset_assignments"`
...
var orderTable = [nCols][4]string{
	...
	{ // granted_by
		"\"granted_by\"",
		"\"granted_by\" DESC",
		"\"granted_by\" ASC NULLS FIRST",
		"\"granted_by\" DESC NULLS LAST",
	},
	{ // granted_at
		"\"granted_at\"",
		"\"granted_at\" DESC",
		"\"granted_at\" ASC NULLS FIRST",
...
var defaultOrder = [1]runtime.Tok{
	runtime.MakeOrder(runtime.Asc, 0), // asset_id
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.Principals, decErr = runtime.TextArray(rv[2], sl)
	if decErr != nil {
		return decErr
	}
	r.GrantedBy = sl.Str(rv[3])
	r.GrantedAt = runtime.Timestamptz(rv[4])
	return decErr
}
...
// No write API: table asset_assignments has no primary key, so no statement can
